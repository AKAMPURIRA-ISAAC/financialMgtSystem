...
def init_app():
    try:
        ...
        health_scorer = FinancialHealthScore()
        return db, ml_engine, analytics, exchange_provider, health_scorer
    except Exception as e:
        st.error(f"Failed to initialize app: {str(e)}")
        return None, None, None, None, None

db, ml_engine, analytics, exchange_provider, health_scorer = init_app()

if not db:
    st.error("❌ Unable to initialize database. Please check the logs.")
    st.stop()
    ...
    try:
        ...
        with col2:
            ...
            try:
                ...
                if monthly_report:
                    fig = go.Figure(data=[
                        go.Bar(name='Income', x=['This Month'], y=[monthly_report.get('summary', {}).get('total_income', 0)]),
                        go.Bar(name='Expenses', x=['This Month'], y=[monthly_report.get('summary', {}).get('total_expenses', 0)])
                    ])
        ...
        try:
            ...
            if report:
                ...
                col1.metric("Total Income", f"UGX {summary.get('total_income', 0):,.0f}")
                col2.metric("Total Expenses", f"UGX {summary.get('total_expenses', 0):,.0f}")
                col3.metric("Net Savings", f"UGX {summary.get('total_income', 0) - summary.get('total_expenses', 0):,.0f}")
        except Exception as e:
